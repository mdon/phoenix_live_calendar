defmodule PhoenixLiveCalendar.Utils.TimeSlots do
  @moduledoc """
  ...
  defp slot_status(date, start_time, _end_time, config, events, now) do
      ...
      true ->
        ...
        booked_count =
          Enum.count(events, fn event ->
            not PhoenixLiveCalendar.Event.all_day?(event) and
              compare_dt(event.start, buffered_end) == :lt and
              compare_dt(PhoenixLiveCalendar.Event.effective_end(event), buffered_start) == :gt
          end)
    ...
    e ->
      Logger.warning(
        "[PhoenixLiveCalendar] Error computing slot status for #{inspect(date)} #{inspect(start_time)}: #{Exception.message(e)}"
      )
