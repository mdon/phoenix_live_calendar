defmodule PhoenixLiveCalendar.Utils.DateHelpers do
  @moduledoc """
  ...
  @spec group_events_by_date([PhoenixLiveCalendar.Event.t()], [Date.t()]) :: %{
          Date.t() => [PhoenixLiveCalendar.Event.t()]
        }
    ...
    do_group = fn ->
      ...
      safe_events =
        Enum.filter(events, fn
          %PhoenixLiveCalendar.Event{id: id, start: start}
          when not is_nil(id) and not is_nil(start) ->
            true

          invalid ->
            Logger.warning(
              "[PhoenixLiveCalendar] Skipping invalid event in grouping: #{inspect(invalid)}"
            )
  ...
  defp put_event_on_date(event, date, acc) do
    if PhoenixLiveCalendar.Event.on_date?(event, date) do
    ...
    e ->
      Logger.warning(
        "[PhoenixLiveCalendar] Error grouping event #{inspect(event.id)}: #{Exception.message(e)}"
      )
