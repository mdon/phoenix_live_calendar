defmodule PhoenixLiveSchedule.PubSub do
  @moduledoc """
  ...
  ### In your LiveView
      ...
      def mount(_params, _session, socket) do
        ...
        PhoenixLiveSchedule.PubSub.subscribe("calendar:my-calendar-id")
        {:ok, socket}
      end

      def handle_info({:phoenix_live_schedule, event, payload}, socket) do
  ...
  ### Broadcasting changes (from your context modules)
      ...
      PhoenixLiveSchedule.PubSub.broadcast("calendar:my-calendar-id", :event_created, new_event)

      # After updating
      PhoenixLiveSchedule.PubSub.broadcast("calendar:my-calendar-id", :event_updated, updated_event)

      # After deleting
      PhoenixLiveSchedule.PubSub.broadcast("calendar:my-calendar-id", :event_deleted, %{id: event_id})

      # Bulk operations — single broadcast
      PhoenixLiveSchedule.PubSub.broadcast("calendar:my-calendar-id", :events_bulk_updated, %{ids: event_ids})

  ## Configuration

  Set the PubSub server in your config (defaults to your app's PubSub):

      config :phoenix_live_schedule, pubsub_server: MyApp.PubSub

  Or pass it explicitly:

      PhoenixLiveSchedule.PubSub.subscribe("topic", pubsub: MyApp.PubSub)
  """
  ...
  The message is sent as `{:phoenix_live_schedule, event_type, payload}`.
  ...
  def broadcast(topic, event_type, payload, opts \\ []) do
    ...
    if pubsub do
      Phoenix.PubSub.broadcast(pubsub, topic, {:phoenix_live_schedule, event_type, payload})
    else
      {:error, :no_pubsub_configured}
    end
  end
  ...
  ## Examples
      ...
      iex> PhoenixLiveSchedule.PubSub.topic("my-calendar")
      "phoenix_live_schedule:my-calendar"

      iex> PhoenixLiveSchedule.PubSub.topic("my-calendar", resource_id: "room-a")
      "phoenix_live_schedule:my-calendar:room-a"
  """
  @spec topic(String.t(), keyword()) :: String.t()
  def topic(calendar_id, opts \\ []) do
    base = "phoenix_live_schedule:#{calendar_id}"
  ...
  defp pubsub_server(opts) do
    Keyword.get(opts, :pubsub) ||
      Application.get_env(:phoenix_live_schedule, :pubsub_server)
  end
end
