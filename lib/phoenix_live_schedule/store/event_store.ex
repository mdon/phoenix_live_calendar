defmodule PhoenixLiveSchedule.Store.EventStore do
  @moduledoc """
  Behaviour defining the data access interface for calendar events.

  Implement this behaviour to provide custom persistence.
  A default Ecto implementation is provided in `PhoenixLiveSchedule.Store.Ecto.EventStoreEcto`.

  ## Example custom implementation

      defmodule MyApp.InMemoryEventStore do
        @behaviour PhoenixLiveSchedule.Store.EventStore
  ...
  Configure your store:
      ...
      config :phoenix_live_schedule, event_store: MyApp.InMemoryEventStore
  """
  ...
  """
  @callback list_events(opts()) :: [PhoenixLiveSchedule.Event.t()]

  @doc "Fetches a single event by ID."
  @callback get_event(event_id(), opts()) :: PhoenixLiveSchedule.Event.t() | nil

  @doc "Creates a new event. Returns `{:ok, event}` or `{:error, changeset}`."
  @callback create_event(map(), opts()) :: {:ok, PhoenixLiveSchedule.Event.t()} | {:error, term()}

  @doc "Updates an existing event."
  @callback update_event(event_id(), map(), opts()) ::
              {:ok, PhoenixLiveSchedule.Event.t()} | {:error, term()}
