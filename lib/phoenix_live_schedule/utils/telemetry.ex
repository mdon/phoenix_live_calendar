defmodule PhoenixLiveSchedule.Utils.Telemetry do
  @moduledoc """
  Performance measurement and telemetry for PhoenixLiveSchedule.
  ...
  All events are prefixed with `[:phoenix_live_schedule]`:

  | Event | Measurements | Metadata |
  |-------|-------------|----------|
  | `[:phoenix_live_schedule, :measure, :start]` | `%{system_time: integer}` | `%{label: atom}` ++ custom |
  | `[:phoenix_live_schedule, :measure, :stop]` | `%{duration: integer}` | `%{label: atom}` ++ custom |
  | `[:phoenix_live_schedule, :measure, :exception]` | `%{duration: integer}` | `%{label: atom, kind: atom, reason: term, stacktrace: list}` |
  | `[:phoenix_live_schedule, :ingress]` | `%{event_count: integer, estimated_bytes: integer}` | `%{view: atom}` |
  ...
  def measure_and_warn(label, metadata \\ %{}, fun) when is_atom(label) and is_function(fun, 0) do
    ...
    :telemetry.execute(
      [:phoenix_live_schedule, :measure, :stop],
  ...
  Emits a `[:phoenix_live_schedule, :ingress]` telemetry event and logs warnings
    ...
    if warnings_enabled?() do
      cond do
        count > @ingress_event_warn_count and estimated_bytes > @ingress_bytes_warn ->
          Logger.warning(
            "[PhoenixLiveSchedule] Large dataset: #{count} events, ~#{format_bytes(estimated_bytes)}. " <>
              "Consider using on_date_range_change to load only the visible range."
          )

        count > @ingress_event_warn_count ->
          Logger.warning(
            "[PhoenixLiveSchedule] #{count} events passed. " <>
              "Consider using on_date_range_change to load only the visible range."
          )

        estimated_bytes > @ingress_bytes_warn ->
          Logger.warning(
            "[PhoenixLiveSchedule] Event data is ~#{format_bytes(estimated_bytes)} (#{count} events). " <>
  ...
  def threshold(label) do
    custom = Application.get_env(:phoenix_live_schedule, :perf_thresholds, %{})
  ...
  def warnings_enabled? do
    Application.get_env(:phoenix_live_schedule, :perf_warnings, true)
  end
  ...
  defp maybe_warn(label, ms, metadata) do
    if warnings_enabled?() and ms > threshold(label) do
      ...
      Logger.warning(
        "[PhoenixLiveSchedule] #{label} took #{Float.round(ms, 1)}ms (threshold: #{threshold(label)}ms). " <>
          "Context: #{meta_str}"
      )
    end
  end
  ...
  defp always_measure? do
    Application.get_env(:phoenix_live_schedule, :perf_always_measure, false)
  end
