defmodule PhoenixLiveSchedule.Utils.Safe do
  @moduledoc false
  ...
  def to_date(other) do
    Logger.warning("[PhoenixLiveSchedule] Cannot convert to Date: #{inspect(other)}")
    nil
  end
  ...
  def to_time(other) do
    Logger.warning("[PhoenixLiveSchedule] Cannot convert to Time: #{inspect(other)}")
    nil
  end
  ...
  def safe_call(fun, fallback \\ nil) do
    ...
    e ->
      Logger.warning("[PhoenixLiveSchedule] Error: #{Exception.message(e)}")
      fallback
  end
  ...
  def ensure_list(other) do
    Logger.warning("[PhoenixLiveSchedule] Expected list, got: #{inspect(other)}")
    []
  end
  ...
  def ensure_pos_integer(val, default) do
    Logger.warning(
      "[PhoenixLiveSchedule] Expected positive integer, got: #{inspect(val)}, using #{default}"
    )
  ...
  def safe_filter_events(events) when is_list(events) do
    Enum.filter(events, fn
      ...
      invalid ->
        Logger.warning("[PhoenixLiveSchedule] Skipping invalid event: #{inspect(invalid)}")
        false
    end)
  end

  def safe_filter_events(other) do
    Logger.warning("[PhoenixLiveSchedule] Expected event list, got: #{inspect(other)}")
    []
  end
