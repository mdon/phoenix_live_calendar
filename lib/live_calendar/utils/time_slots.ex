defmodule LiveCalendar.Utils.TimeSlots do
  @moduledoc """
  ...
  alias LiveCalendar.{Availability, BookingConfig}
  ...
  defp slot_status(date, start_time, _end_time, config, events, now) do
      ...
      true ->
        ...
        booked_count =
          Enum.count(events, fn event ->
            not LiveCalendar.Event.all_day?(event) and
              compare_dt(event.start, buffered_end) == :lt and
              compare_dt(LiveCalendar.Event.effective_end(event), buffered_start) == :gt
          end)
    ...
    e ->
      Logger.warning(
        "[LiveCalendar] Error computing slot status for #{inspect(date)} #{inspect(start_time)}: #{Exception.message(e)}"
      )
