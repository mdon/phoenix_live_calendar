defmodule LiveCalendar.Utils.DateHelpers do
  @moduledoc """
  ...
  """
  @spec group_events_by_date([LiveCalendar.Event.t()], [Date.t()]) :: %{
          Date.t() => [LiveCalendar.Event.t()]
        }
    ...
    do_group = fn ->
      ...
      safe_events =
        Enum.filter(events, fn
          ...
          invalid ->
            Logger.warning("[LiveCalendar] Skipping invalid event in grouping: #{inspect(invalid)}")
            false
        end)
      ...
      Enum.reduce(safe_events, base, fn event, acc ->
        Enum.reduce(dates, acc, fn date, inner_acc ->
          try do
            if LiveCalendar.Event.on_date?(event, date) do
            ...
            e ->
              Logger.warning(
                "[LiveCalendar] Error grouping event #{inspect(event.id)}: #{Exception.message(e)}"
              )
