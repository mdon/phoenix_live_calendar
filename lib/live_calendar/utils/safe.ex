defmodule LiveCalendar.Utils.Safe do
  @moduledoc false
  ...
  def to_date(other) do
    Logger.warning("[LiveCalendar] Cannot convert to Date: #{inspect(other)}")
    nil
  end
  ...
  def to_time(other) do
    Logger.warning("[LiveCalendar] Cannot convert to Time: #{inspect(other)}")
    nil
  end
  ...
  def safe_call(fun, fallback \\ nil) do
    ...
    e ->
      Logger.warning("[LiveCalendar] Error: #{Exception.message(e)}")
      fallback
  end
  ...
  def ensure_list(other) do
    Logger.warning("[LiveCalendar] Expected list, got: #{inspect(other)}")
    []
  end
  ...
  def ensure_pos_integer(val, default) do
    Logger.warning(
      "[LiveCalendar] Expected positive integer, got: #{inspect(val)}, using #{default}"
    )
  ...
  def sanitize_css_dimension(value, _fallback) when is_binary(value) do
    if Regex.match?(~r/^\d+(\.\d+)?\s*(px|rem|em|vh|vw|%|ch|ex|vmin|vmax)$/, value) do
      ...
      Logger.warning("[LiveCalendar] Invalid CSS dimension: #{inspect(value)}, using fallback")
      "3rem"
    end
  end
  ...
  def safe_filter_events(events) when is_list(events) do
    Enum.filter(events, fn
      %LiveCalendar.Event{id: id, start: start} when not is_nil(id) and not is_nil(start) ->
        true

      invalid ->
        Logger.warning("[LiveCalendar] Skipping invalid event: #{inspect(invalid)}")
        false
    end)
  end

  def safe_filter_events(other) do
    Logger.warning("[LiveCalendar] Expected event list, got: #{inspect(other)}")
    []
  end
