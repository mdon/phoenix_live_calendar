defmodule LiveCalendar.Utils.Telemetry do
  @moduledoc """
  Performance measurement and telemetry for LiveCalendar.
  ...
  All events are prefixed with `[:live_calendar]`:

  | Event | Measurements | Metadata |
  |-------|-------------|----------|
  | `[:live_calendar, :measure, :start]` | `%{system_time: integer}` | `%{label: atom}` ++ custom |
  | `[:live_calendar, :measure, :stop]` | `%{duration: integer}` | `%{label: atom}` ++ custom |
  | `[:live_calendar, :measure, :exception]` | `%{duration: integer}` | `%{label: atom, kind: atom, reason: term, stacktrace: list}` |
  | `[:live_calendar, :ingress]` | `%{event_count: integer, estimated_bytes: integer}` | `%{view: atom}` |
  ...
  Emits a `[:live_calendar, :ingress]` telemetry event and logs warnings
    ...
    if warnings_enabled?() do
      cond do
        count > @ingress_event_warn_count and estimated_bytes > @ingress_bytes_warn ->
          Logger.warning(
            "[LiveCalendar] Large dataset: #{count} events, ~#{format_bytes(estimated_bytes)}. " <>
              "Consider using on_date_range_change to load only the visible range."
          )

        count > @ingress_event_warn_count ->
          Logger.warning(
            "[LiveCalendar] #{count} events passed. " <>
              "Consider using on_date_range_change to load only the visible range."
          )

        estimated_bytes > @ingress_bytes_warn ->
          Logger.warning(
            "[LiveCalendar] Event data is ~#{format_bytes(estimated_bytes)} (#{count} events). " <>
  ...
  def threshold(label) do
    custom = Application.get_env(:live_calendar, :perf_thresholds, %{})
  ...
  def warnings_enabled? do
    Application.get_env(:live_calendar, :perf_warnings, true)
  end
  ...
  defp maybe_warn(label, ms, metadata) do
    if warnings_enabled?() and ms > threshold(label) do
      ...
      Logger.warning(
        "[LiveCalendar] #{label} took #{Float.round(ms, 1)}ms (threshold: #{threshold(label)}ms). " <>
          "Context: #{meta_str}"
      )
    end
  end
  ...
  defp always_measure? do
    Application.get_env(:live_calendar, :perf_always_measure, false)
  end
