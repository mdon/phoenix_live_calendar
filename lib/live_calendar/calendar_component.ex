defmodule LiveCalendar.CalendarComponent do
  @moduledoc """
  ...
  unless Application.compile_env(:live_calendar, :skip_install_check, false) do
    unless LiveCalendar.installed?() do
      IO.warn("""
      LiveCalendar CSS integration not detected.

      Tailwind will not scan LiveCalendar's component templates, so styles
      ...
      To suppress this warning, add to your config:
          ...
          config :live_calendar, skip_install_check: true
      """)
    end
  end

  alias LiveCalendar.Event
  alias LiveCalendar.Utils.{DateHelpers, I18n, Safe, Telemetry}

  alias LiveCalendar.Views.{
    Agenda,
    DayView,
    MonthGrid,
    NDayView,
  ...
  alias LiveCalendar.Components.Header
  ...
  def render(assigns) do
    ...
    <div
      ...
      phx-hook={if(assigns[:enable_hooks], do: "LiveCalendarContainer")}
    >
      <Header.header
  ...
  def handle_event("lc_view_change", %{"view" => view_str}, socket) do
    case safe_parse_view(view_str) do
      nil ->
        Logger.warning("[LiveCalendar] Invalid view: #{inspect(view_str)}")
        {:noreply, socket}
  ...
  def handle_event("lc_date_click", %{"date" => date_str}, socket) when is_binary(date_str) do
    case Date.from_iso8601(date_str) do
      ...
      _ ->
        Logger.warning("[LiveCalendar] Invalid date in lc_date_click: #{inspect(date_str)}")
        {:noreply, socket}
    end
  end
  ...
  def handle_event("lc_time_click", params, socket) when is_map(params) do
    with date_str when is_binary(date_str) <- params["date"],
      ...
      _ ->
        Logger.warning("[LiveCalendar] Invalid params in lc_time_click: #{inspect(params)}")
        {:noreply, socket}
    end
  end
  ...
  def handle_event("lc_more_click", %{"date" => date_str}, socket) when is_binary(date_str) do
    case Date.from_iso8601(date_str) do
      ...
      _ ->
        Logger.warning("[LiveCalendar] Invalid date in lc_more_click: #{inspect(date_str)}")
        {:noreply, socket}
    end
  end
  ...
  def handle_event("lc_range_select", params, socket) when is_map(params) do
    with date_str when is_binary(date_str) <- params["date"],
      ...
      _ -> Logger.warning("[LiveCalendar] Invalid params in lc_range_select: #{inspect(params)}")
    end
  ...
  def handle_event(event_name, params, socket) do
    Logger.warning(
      "[LiveCalendar] Unhandled event: #{inspect(event_name)}, params: #{inspect(params)}"
    )
  ...
  defp safe_direction(other) do
    Logger.warning("[LiveCalendar] Invalid direction: #{inspect(other)}, defaulting to :next")
    :next
  end
    ...
    case Map.get(socket.assigns, callback_key) do
      ...
      callback when is_function(callback) ->
        try do
          ...
          e ->
            Logger.warning(
              "[LiveCalendar] Callback #{callback_key} raised: #{Exception.message(e)}"
            )
        end

      other ->
        Logger.warning(
          "[LiveCalendar] Expected function for #{callback_key}, got: #{inspect(other)}"
        )
    end
  end
