if Code.ensure_loaded?(Ecto) do
  defmodule LiveCalendar.Store.Ecto.Migrations do
    @moduledoc """
    Versioned migrations for LiveCalendar's Ecto persistence layer.

    Follows the Oban pattern: consumers generate a migration that delegates
    to this module, allowing LiveCalendar to manage schema changes across
    ...
    defp migrate_v1_up(prefix) do
      create_if_not_exists table(:live_calendar_events, primary_key: false, prefix: prefix) do
      ...
      # Performance indexes
      create_if_not_exists(index(:live_calendar_events, [:start_at], prefix: prefix))
      create_if_not_exists(index(:live_calendar_events, [:calendar_id, :start_at], prefix: prefix))
      create_if_not_exists(index(:live_calendar_events, [:resource_id, :start_at], prefix: prefix))

      create_if_not_exists(
        index(:live_calendar_events, [:calendar_id, :resource_id, :start_at], prefix: prefix)
      )

      create_if_not_exists(index(:live_calendar_events, [:group_id], prefix: prefix))
      create_if_not_exists(index(:live_calendar_events, [:recurrence_id], prefix: prefix))

      # Resource table
      create_if_not_exists table(:live_calendar_resources, primary_key: false, prefix: prefix) do
        add(:id, :binary_id, primary_key: true)
      ...
      create_if_not_exists(index(:live_calendar_resources, [:calendar_id], prefix: prefix))
      create_if_not_exists(index(:live_calendar_resources, [:parent_id], prefix: prefix))
    end

    defp migrate_v1_down(prefix) do
      drop_if_exists(table(:live_calendar_resources, prefix: prefix))
      drop_if_exists(table(:live_calendar_events, prefix: prefix))
    end
  end
end
