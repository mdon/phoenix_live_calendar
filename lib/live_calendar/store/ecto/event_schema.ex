if Code.ensure_loaded?(Ecto) do
  defmodule LiveCalendar.Store.Ecto.EventSchema do
    @moduledoc """
    Default Ecto schema for calendar events.

    This schema is provided as a convenience for consumers who opt into
    the Ecto persistence layer. It maps directly to `LiveCalendar.Event`.

    ## Table name

    Defaults to `live_calendar_events`. Configure with:

        config :live_calendar, table_prefix: "my_prefix"
    ...
    schema "live_calendar_events" do
      field(:title, :string)
    ...
    @doc "Converts this Ecto schema to a `LiveCalendar.Event` struct."
