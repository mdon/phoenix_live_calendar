defmodule Mix.Tasks.PhoenixLiveSchedule.InstallTest do
  # async: false + @tag :tmp_dir — swaps the global Mix.shell and writes fixture files.
  use ExUnit.Case, async: false

  @task Mix.Tasks.PhoenixLiveSchedule.Install
  ...
  test "injects the CSS @source and the JS import + hooks into a fresh project", %{tmp_dir: tmp} do
    ...
    assert css_out =~ "PhoenixLiveSchedule CSS Integration"
    assert css_out =~ ~s(@source "../../deps/phoenix_live_schedule")

    js_out = File.read!(js)
    assert js_out =~ "PhoenixLiveSchedule JS hooks"
    assert js_out =~ "deps/phoenix_live_schedule/priv/static/assets/phoenix_live_schedule.js"
    assert js_out =~ "...window.PhoenixLiveScheduleHooks"
  end
  ...
  test "leaves hooks wiring to the user when it can't be done safely", %{tmp_dir: tmp} do
    ...
    js_out = File.read!(js)
    assert js_out =~ "phoenix_live_schedule.js"
    refute js_out =~ "...window.PhoenixLiveScheduleHooks"
  ...
  test "prints manual instructions when no css/js file is found", %{tmp_dir: tmp} do
    ...
    assert out =~ ~s(@source "../../deps/phoenix_live_schedule")
    assert out =~ "Could not find app.js"
    assert out =~ "...window.PhoenixLiveScheduleHooks"
  end
end
