defmodule LiveCalendarTest do
  use ExUnit.Case

  alias LiveCalendar.{Availability, BookingConfig, Event, Resource}

  describe "event/3" do
    test "creates an event with required fields" do
      event = LiveCalendar.event("1", ~D[2026-04-01])
      assert %Event{id: "1", start: ~D[2026-04-01]} = event
    end

    test "creates an event with optional fields" do
      event =
        LiveCalendar.event("1", ~U[2026-04-01 10:00:00Z], title: "Meeting", color: "bg-primary")
  ...
  describe "resource/3" do
    test "creates a resource" do
      resource = LiveCalendar.resource("room-a", "Conference Room A")
  ...
  describe "availability/4" do
    test "creates recurring availability" do
      avail = LiveCalendar.availability([1, 2, 3, 4, 5], ~T[09:00:00], ~T[17:00:00])
      assert %Availability{days_of_week: [1, 2, 3, 4, 5]} = avail
    end

    test "creates date-specific availability" do
      avail = LiveCalendar.availability(~D[2026-04-15], ~T[10:00:00], ~T[14:00:00])
  ...
  describe "to_events/1" do
    test "passes through Event structs" do
      events = [LiveCalendar.event("1", ~D[2026-04-01])]
      assert [%Event{id: "1"}] = LiveCalendar.to_events(events)
    end
  end
end
