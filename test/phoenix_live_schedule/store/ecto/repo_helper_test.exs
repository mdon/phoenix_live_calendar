...
defmodule PhoenixLiveSchedule.Store.Ecto.RepoHelperTest do
  # async: false — these mutate the global :phoenix_live_schedule, :repo app env.
  use ExUnit.Case, async: false

  alias PhoenixLiveSchedule.Store.Ecto.RepoHelper
  ...
  describe "repo/0" do
    test "returns the configured repo module" do
      Application.put_env(:phoenix_live_schedule, :repo, RepoHelperFakeRepo)
      assert RepoHelper.repo() == RepoHelperFakeRepo
    end

    test "raises a helpful error when no repo is configured" do
      Application.delete_env(:phoenix_live_schedule, :repo)
