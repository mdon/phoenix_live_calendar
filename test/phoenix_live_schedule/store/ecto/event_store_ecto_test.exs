defmodule EventStoreEctoFakeRepo do
  ...
  alias PhoenixLiveSchedule.Store.Ecto.EventSchema
...
defmodule PhoenixLiveSchedule.Store.Ecto.EventStoreEctoTest do
  # async: false — mutates the global :phoenix_live_schedule, :repo app env.
  use ExUnit.Case, async: false

  alias PhoenixLiveSchedule.Event
  alias PhoenixLiveSchedule.Store.Ecto.{EventSchema, EventStoreEcto}
  ...
  setup do
    prev = Application.get_env(:phoenix_live_schedule, :repo)
    Application.put_env(:phoenix_live_schedule, :repo, EventStoreEctoFakeRepo)
    Process.put(:fake_schema, @schema)

    on_exit(fn ->
      if prev,
        do: Application.put_env(:phoenix_live_schedule, :repo, prev),
        else: Application.delete_env(:phoenix_live_schedule, :repo)
    end)
