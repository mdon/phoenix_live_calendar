defmodule PhoenixLiveCalendar.Utils.TelemetryTest do
  ...
  setup do
    ...
    prev_warnings = Application.get_env(:phoenix_live_calendar, :perf_warnings)
    prev_thresholds = Application.get_env(:phoenix_live_calendar, :perf_thresholds)
    prev_always = Application.get_env(:phoenix_live_calendar, :perf_always_measure)
  ...
  defp restore_config(key, nil), do: Application.delete_env(:phoenix_live_calendar, key)
  defp restore_config(key, val), do: Application.put_env(:phoenix_live_calendar, key, val)
  ...
  describe "measure_and_warn/3" do
    ...
    test "logs warning when threshold is exceeded" do
      Application.put_env(:phoenix_live_calendar, :perf_thresholds, %{slow_test: 0})
      Application.put_env(:phoenix_live_calendar, :perf_warnings, true)
      ...
      assert log =~ "[PhoenixLiveCalendar] slow_test took"
      assert log =~ "threshold: 0ms"
      assert log =~ "items: 5"
    end

    test "does not log when under threshold" do
      Application.put_env(:phoenix_live_calendar, :perf_thresholds, %{fast_test: 5000})
    ...
    test "respects perf_warnings: false" do
      Application.put_env(:phoenix_live_calendar, :perf_warnings, false)
      Application.put_env(:phoenix_live_calendar, :perf_thresholds, %{silent_test: 0})
  ...
  describe "should_measure?/1" do
    ...
    test "respects perf_always_measure config" do
      Application.put_env(:phoenix_live_calendar, :perf_always_measure, true)
  ...
  describe "threshold/1" do
    ...
    test "respects custom threshold config" do
      Application.put_env(:phoenix_live_calendar, :perf_thresholds, %{group_events: 50})
  ...
  describe "warnings_enabled?/0" do
    test "returns true by default" do
      Application.delete_env(:phoenix_live_calendar, :perf_warnings)
