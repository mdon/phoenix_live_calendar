 ...
 *     hooks: { ...window.LiveCalendarHooks, ...myHooks }
 *   })
 */
...
(function () {
  ...
  window.LiveCalendarHooks = window.LiveCalendarHooks || {};

  // ============================================================
  // TimeRangeSelect — drag to select a time range on a time grid
  // ============================================================
  window.LiveCalendarHooks.TimeRangeSelect = {
    mounted() {
  ...
  // ============================================================
  window.LiveCalendarHooks.EventDrag = {
    mounted() {
      this._dragging = null;
      this._ghost = null;
      this._startX = 0;
  ...
  // ============================================================
  window.LiveCalendarHooks.EventResize = {
    mounted() {
      this._resizing = null;
  ...
  // ============================================================
  window.LiveCalendarHooks.ResponsiveContainer = {
    mounted() {
  ...
  // ============================================================
  window.LiveCalendarHooks.TouchHandler = {
    mounted() {
  ...
  // ============================================================
  window.LiveCalendarHooks.PopoverPause = {
    mounted() {
  ...
  // ============================================================
  window.LiveCalendarHooks.MarkerTicker = {
    mounted() {
  ...
  // ============================================================
  // LiveCalendarContainer — composite hook for the main container
  // ============================================================
  window.LiveCalendarHooks.LiveCalendarContainer = {
    mounted() {
      ...
      hooks.forEach((name) => {
        var hook = Object.create(window.LiveCalendarHooks[name]);
        hook.el = this.el;
  ...
  window.LiveCalendarHooks.WaterfallAutoScroll = {
    mounted() {
  ...
  // pointer handling.
  window.LiveCalendarHooks.WaterfallBarPopover = {
    mounted() {
      this._onClick = (e) => {
      ...
      // them in one pass.
      window.LiveCalendarHooks.WaterfallBarPopover._installGlobal();
      window.LiveCalendarHooks.WaterfallBarPopover._bars.add(this.el);
    },

    destroyed() {
      this.el.removeEventListener("click", this._onClick);
      window.LiveCalendarHooks.WaterfallBarPopover._bars.delete(this.el);
      this._close();
    },
    ...
    _open() {
      const p = this._popover();
      if (!p) return;
      window.LiveCalendarHooks.WaterfallBarPopover._closeAll();
      ...
      if (eventId) {
        window.LiveCalendarHooks.WaterfallBarPopover._applyTreeFade(this.el, eventId);
      }
      ...
      requestAnimationFrame(() => {
        window.LiveCalendarHooks.WaterfallBarPopover._pushBottomBadges(this.el, p);
      });
    },
    ...
    _close() {
      ...
      window.LiveCalendarHooks.WaterfallBarPopover._clearTreeFade(this.el);
      window.LiveCalendarHooks.WaterfallBarPopover._restoreBottomBadges(this.el);
    },
  ...
  window.LiveCalendarHooks.WaterfallBarPopover._bars = new Set();
  window.LiveCalendarHooks.WaterfallBarPopover._globalInstalled = false;

  window.LiveCalendarHooks.WaterfallBarPopover._installGlobal = function () {
  ...
  window.LiveCalendarHooks.WaterfallBarPopover._closeAll = function () {
    this._bars.forEach((bar) => {
      if (bar.dataset.popoverOpen !== "true") return;
      const popoverId = bar.dataset.popoverTarget;
      const popover = popoverId ? document.getElementById(popoverId) : null;
  ...
  window.LiveCalendarHooks.WaterfallBarPopover._collectTree = function (chartEl, activeId) {
  ...
  window.LiveCalendarHooks.WaterfallBarPopover._applyTreeFade = function (activeEl, activeId) {
  ...
  window.LiveCalendarHooks.WaterfallBarPopover._clearTreeFade = function (activeEl) {
    const chartEl = activeEl.closest(".cal-waterfall-wrap");
    if (!chartEl) return;
    chartEl
  ...
  window.LiveCalendarHooks.WaterfallBarPopover._pushBottomBadges = function (activeEl, popover) {
    const chartEl = activeEl.closest(".cal-waterfall-wrap");
    if (!chartEl) return;
  ...
  window.LiveCalendarHooks.WaterfallBarPopover._restoreBottomBadges = function (activeEl) {
    const chartEl = activeEl.closest(".cal-waterfall-wrap");
    if (!chartEl) return;
  ...
  // Log initialization
  var hookCount = Object.keys(window.LiveCalendarHooks).length;
  if (typeof console !== "undefined" && console.debug) {
    console.debug(
      "[LiveCalendar] Initialized with " + hookCount + " hook(s):",
      Object.keys(window.LiveCalendarHooks)
    );
  }
})();
